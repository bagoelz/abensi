...
export class MasukComponent implements OnInit {
    ...
    this.API.getAbsensiMasuk(this.token).subscribe(result => {
      ...
      this.fullAbsen.forEach(item => {
        this.ListCetak.push({
            ...
            absenmasuk:item.jam_masuk,
            absenkeluar:item.jam_keluar,
            istirahatKeluar:item.istirahat_keluar,
            istirahatMasuk:item.istirahat_masuk,
            fotoistirahatK:item.fotoIstirahatK,
            fotoistirahatM:item.fotoIstirahatM,
        })
      });
  ...
  onChange(e) {
    this.ListAbsen = [];
    const _ = moment();
    let tmpselected = moment(new Date()).format('DD MMMM YYYY').toString();
    if (e === 'null') {
      // setTimeout(() => {
      this.fullAbsen.forEach(item => {
        if (item.tanggal === tmpselected) {
  ...
  generateExcel(name?: string) {
    let timeSpan = new Date().toISOString();
    let prefix = name || "ExportResult";
    let fileName = `${prefix}-${timeSpan}`;
    const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.ListCetak);
...
export class DetailAbsensi {
  ...
  OnUpdate() {
    this.API.UpdateStatus(this.data.idKeluar, this.data.status, (new Date(this.data.tanggal).getTime() / 1000)).subscribe(result => {
      const Status = result.status;
      if (Status === 0) {
        this.dialogRef.close({ event: 'cancel' });
      } else {
